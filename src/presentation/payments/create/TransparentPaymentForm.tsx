...
export default function TransparentPaymentForm({ amount, appointmentId, userEmail }: TransparentPaymentFormProps) {
    ...
    const [pixData, setPixData] = useState<{ qrCode: string; ticket: string; paymentId: string } | null>(null);
    ...
    useEffect(() => {
        ...
        intervalRef.current = setInterval(async () => {
            try {
                ...
                setIsChecking(true);
                const res = await fetch(`/api/payments/check-status?id=${pixData.paymentId}`);
                const data = await res.json();
                setIsChecking(false);

                if (data.status === "CONFIRMED") {
                    stopPolling();
                    router.push(`/success?payment_id=${pixData.paymentId}`);
            ...
            {pixData ? (
                <div className="flex flex-col items-center space-y-6 animate-in fade-in duration-500">
                    ...
                    <div className="text-xs text-center text-gray-400">
                        <p>Verificando pagamento a cada 5 segundos.</p>
                        <p>Tempo restante: expira em 5 minutos.</p>
                    </div>
