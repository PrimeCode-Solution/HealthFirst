  ...
  try {
    ...
    if (status) {
      ...
      if (!allowedStatuses.includes(status)) {
        return NextResponse.json({ error: "invalid_status", allowed: allowedStatuses }, { status: 400 })
      }
    ...
    const [total, items] = await Promise.all([
      ...
      prisma.appointment.findMany({
        ...
        select: {
          ...
          updatedAt: true,
          payment: {
            select: { id: true, status: true, amount: true, currency: true, preferenceId: true, paidAt: true },
          },
        },
      }),
    ])
    ...
    return NextResponse.json({ meta: { total, page: p, pageSize: ps, pageCount }, items })
  } catch (e) {
    console.error(e)
